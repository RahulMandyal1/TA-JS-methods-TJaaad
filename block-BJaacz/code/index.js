...
function addTwoToArray(arr){
   let finalArray =[];
  //  for (let nummber of arr){
  //    finalArray.push(arr[nummber+2]);
  //  }
  for(let element = 0 ; element <= arr.length-1 ; element++){
    finalArray.push(arr[element]+2);
  }
   return finalArray;
}
...
function changeArray(arr , cb){
  let finalArray = [];
  for(let element = 0; element <= arr.length-1 ; element++){
    finalArray.push( arr[element]+cb())
  }
  return finalArray;
}
// add Two three Five is already defiend so i have used addTwoNumbers , addThreeNumber , addFiveNumber
...
function filterGradeB(arr){
  let finalArray = [];
  for( let  i =0; i< arr.length ; i++){
    if( arr[i].grade > 8 && arr[i].grade<=12){
     finalArray.push(arr[i]);
    }
  }
  return finalArray;
...
*/
function  filter( arr ,cb){ // higher order function ;
  let cbargument = arr;
  let finalArray = [];
  for(let i = 0 ; i < arr.length ; i++){
    if(cb(cbargument ,i)){
...
// Grade  A  grade  is gretaer than  12
...
// Grade  B logic  grade is  greater  than 8 but  smaller  than equal to  12
...
// Grade  c logic  grade is smaller than equal to 8
let  isGradeC= (arr ,i) =>{
  if(arr[i].grade <=8){
    return true;
   }
